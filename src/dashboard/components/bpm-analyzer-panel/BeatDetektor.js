...
BeatDetektor.prototype.reset = function() {
  ...
  for (var i = 0; i < this.config.BD_DETECTION_RANGES; i++) {
    ...
    this.ma_bpm_range[i] = this.maa_bpm_range[i] =
      60.0 / this.BPM_MIN + (60.0 / this.BPM_MAX - 60.0 / this.BPM_MIN) * (i / this.config.BD_DETECTION_RANGES)
...
BeatDetektor.prototype.process = function(timer_seconds, fft_data) {
  ...
  for (x = 0; x < fft_data.length; x += range_step) {
    ...
    this.ma_freq_range[range] -=
      (this.ma_freq_range[range] - this.a_freq_range[range]) * this.last_update * this.config.BD_DETECTION_RATE
    ...
    this.maa_freq_range[range] -=
      (this.maa_freq_range[range] - this.ma_freq_range[range]) * this.last_update * this.config.BD_DETECTION_RATE
      ...
      if (rewarded) {
        ...
        if (this.detection_quality[range] < this.quality_avg * this.config.BD_QUALITY_TOLERANCE && this.current_bpm) {
          ...
          this.maa_bpm_range[range] -=
            (this.maa_bpm_range[range] - this.ma_bpm_range[range]) * this.config.BD_QUALITY_STEP
        }
    ...
    if (
      ...
      this.detection_quality[range] -=
        this.detection_quality[range] * this.config.BD_QUALITY_STEP * this.config.BD_QUALITY_DECAY * this.last_update
  ...
  if (this.quality_total) {
    ...
    this.ma_quality_avg +=
      ((this.quality_avg - this.ma_quality_avg) * this.last_update * this.config.BD_DETECTION_RATE) / 2.0

    this.maa_quality_avg += (this.ma_quality_avg - this.maa_quality_avg) * this.last_update
    this.ma_quality_total +=
      ((this.quality_total - this.ma_quality_total) * this.last_update * this.config.BD_DETECTION_RATE) / 2.0
  ...
  for (var i = 0; i < det.config.BD_DETECTION_RANGES; i++) {
    ...
    if (det_val > 1.0) {
      ...
      if (det_val > this.vu_levels[i]) this.vu_levels[i] = det_val
        ...
        this.vu_levels[i] -= (this.vu_levels[i] - det_val) * det.last_update * (1.0 / det.current_bpm) * 3.0
    } else {
